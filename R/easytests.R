...
test_expected_rows <- function(df, expected_rows) {
  testthat::test_that(paste0("Number of rows in dataframe [", deparse(substitute(df)), "] is (", expected_rows,")"),
...
test_column_sum <- function(df, col, expected_sum) {
  testthat::test_that(paste0("Sum of column [", deparse(substitute(col)) ,"] in dataframe [", deparse(substitute(df)), "] is (", expected_sum,")"),
...
test_column_unique_vals <- function(df, col, expected_unique_vals) {
  testthat::test_that(paste0("Number of unique values in column [", deparse(substitute(col)) ,"] in dataframe [", deparse(substitute(df)), "] is (", expected_unique_vals,")"),
